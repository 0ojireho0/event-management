...
export function getCheckInFeedback(errorOrResponse) {
  ...
  if (status === 409) return { tone: "warning", title: "Already checked in" };
  if (status === 404) return { tone: "error", title: "Registration not found" };
...
export function shouldPauseCamera({ submitting, scannedCode }) {
  return Boolean(submitting || scannedCode);
}
